...
// CRuleGrid.h : interface of the CRuleGrid class
//
...
const int WM_RULE_GRID_CELL_VALUE_CHANGE	= WM_USER + 101;
const int WM_RULE_GRID_DBLCLICK				= WM_USER + 102;
const int WM_RULE_GRID_SELCHANGE			= WM_USER + 103;
const int WM_RULE_GRID_RCLICK				= WM_USER + 104;

class CRuleGrid:public CUGCtrl
{
public:
	CRuleGrid();
	~CRuleGrid();
...
public:
	...
	void SetRowInfo(int nRow, bool bEnabled, bool bIgnoreError, const CString& zDescription);

	// SNK 7/23/2011
	// Gets/set the checkbox in the "Run" column of the table.
	bool GetEnabled(int nRowIndex);
	void SetEnabled(int nRowIndex, bool bEnabled);

	// SNK 7/23/2011
	// Gets/set the checkbox in the "Run" column of the table.
	bool GetIgnoreError(int nRowIndex);
	void SetIgnoreError(int nRowIndex, bool bIgnoreError);

	// SNK 7/23/2011
	// Gets/set the text in the "Task" column of the table.
	CString GetDescription(int nRowIndex);
	void SetDescription(int nRowIndex, CString zDescription);
