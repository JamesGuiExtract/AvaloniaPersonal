...
nmake /X "%TEMP%\nmakeErrors" /F AttributeFinderSDK.mak Branch=%Branch% BuildConfig="Release" ProductRootDirName="%PRODUCT_ROOT%" ProductVersion="%~1" %BuildScriptTarget% 2>&1 | tee "%LOGFILE%"
IF EXIST "%TEMP%\nmakeErrors" (
...
IF EXIST "%TEMP%\nmakeErrors" del "%TEMP%\nmakeErrors"
nmake /X "%TEMP%\nmakeErrors" /F RuleDevelopmentKit.mak Branch=%Branch% BuildConfig="Release" ProductRootDirName="%PRODUCT_ROOT%" ProductVersion="%~1" DoEverything 2>&1 | tee "%LOGFILE2%"
IF EXIST "%TEMP%\nmakeErrors" (
	FIND "NMAKE : fatal error" "%TEMP%\nmakeErrors"
:: If there were no errors nothing will be found and FIND will return an errorlevel of 1
	IF NOT ERRORLEVEL 1 (
