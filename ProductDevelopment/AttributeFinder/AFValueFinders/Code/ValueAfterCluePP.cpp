  ...
  m_bIncludeClueLine(false)
{
	try
	{
		...
		m_ipRegExpParser = ipMiscUtils->GetNewRegExpParserInstance("ValueAfterClue");
		ASSERT_RESOURCE_ALLOCATION("ELI04586", m_ipRegExpParser != NULL);
	}
...
//-------------------------------------------------------------------------------------------------
LRESULT CValueAfterCluePP::OnClickedSeperateCharInfo(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
