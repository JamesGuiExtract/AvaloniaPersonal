...
LRESULT CREPMFinderPP::OnClickedPatternFileInfo(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
{
	...
	try
	{
		...
		CString zText("Provide an existing file name or an unspecified file name,\n"
					  ...
					  "...\n"
					  "Note:\n"
					  "- To include another file, start a line with \"#import\". The file name that follows should be relative to the dat file location.\n"
					  "- Imported files are evaluated before evaluating patterns, so imports may specify a regex to be inserted into the current pattern.\n"
					  "- Each pattern declaration must have a unique pattern ID enclosed in angle brackets, followed by three equals signs, followed by the pattern text.\n"
					  "  No whitespace is allowed on the left side of the equals sign. Any white presented to the right of the equals sign will be considered as a\n"
					  "  part of the pattern text.\n"
					  "- Pattern ID can be composed of any characters, including whitespace characters. There's no upper limit on number of characters to form a pattern ID\n"
					  ...
					  "- If the pattern matches, each named group from the regex will generate an attribute with a type matching the name of the group. (The overall match\n"
