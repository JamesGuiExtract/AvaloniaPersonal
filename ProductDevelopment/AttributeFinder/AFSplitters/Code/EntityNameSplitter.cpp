  ...
  m_bMoveTrustName(false)
{
	try
	{
		...
		ASSERT_RESOURCE_ALLOCATION("ELI22438", ipMiscUtils != NULL );

		// Get a regular expression parser
		m_ipRegExprParser = ipMiscUtils->GetNewRegExpParserInstance("EntityNameSplitter");
		ASSERT_RESOURCE_ALLOCATION( "ELI22439", m_ipRegExprParser != NULL );
	}
													 ...
													 IProgressStatus *pProgressStatus)
{
	try
	{
		...
		if (bIsTrust)
		{
			...
			bIsTrust = doTrustSplitting( ipEntity, pAFDoc, ipMainAttrSub );
			if (bIsTrust)
			{
				return S_OK;
			}
				...
				if (isValidEntity( ipPerson, true ))
				{
					...
					while (true)
					{
						...
						if (isValidEntity( ipPerson, true ))
						{
							...
							if (bSplitPerson)
							{
								ipPersonSplitter->SplitAttribute( ipAttr, pAFDoc, NULL );
							}
							lPersonCount++;
