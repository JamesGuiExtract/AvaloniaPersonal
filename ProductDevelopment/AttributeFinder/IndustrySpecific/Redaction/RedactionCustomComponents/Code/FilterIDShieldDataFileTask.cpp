...
//--------------------------------------------------------------------------------------------------
STDMETHODIMP CFilterIDShieldDataFileTask::get_DisplaysUI(VARIANT_BOOL *pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	try
	{
		ASSERT_ARGUMENT("ELI44989", pVal != __nullptr);
	...
	CATCH_ALL_AND_RETURN_AS_COM_ERROR("ELI44990");
}
...
STDMETHODIMP CFilterIDShieldDataFileTask::raw_RequiresAdminAccess(VARIANT_BOOL* pbResult)
{
	...
	try
	{
		ASSERT_ARGUMENT("ELI31195", pbResult != __nullptr);

		*pbResult = VARIANT_FALSE;

		return S_OK;
	}
	CATCH_ALL_AND_RETURN_AS_COM_ERROR("ELI31196");
}
