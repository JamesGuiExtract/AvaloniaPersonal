...
//-------------------------------------------------------------------------------------------------
STDMETHODIMP CCleanupImageFileProcessor::raw_Standby(VARIANT_BOOL* pVal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	
	try
	{		
		ASSERT_ARGUMENT("ELI33898", pVal != __nullptr);

		*pVal = VARIANT_TRUE;

		return S_OK;
	}
	CATCH_ALL_AND_RETURN_AS_COM_ERROR("ELI33899");
}
//--------------------------------------------------------------------------------------------------
STDMETHODIMP CCleanupImageFileProcessor::get_MinStackSize(unsigned long *pnMinStackSize)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	try
	{
		ASSERT_ARGUMENT("ELI35005", pnMinStackSize != __nullptr);
...
STDMETHODIMP CCleanupImageFileProcessor::raw_GetComponentDescription(BSTR * pstrComponentDescription)
{
	...
	try
	{
		...
		*pstrComponentDescription = _bstr_t("Core: Clean up image").Detach();
	}
