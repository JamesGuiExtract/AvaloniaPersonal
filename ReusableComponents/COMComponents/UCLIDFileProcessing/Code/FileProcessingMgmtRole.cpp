...
WorkItemThreadData::WorkItemThreadData(CFileProcessingMgmtRole* pFPMgmtRole, long nActionID, 
	Win32Semaphore &rSemaphore, IFileProcessingDB *pDB, bool bProcessForCurrentUPIOnly) :
m_pFPMgmtRole(pFPMgmtRole),
m_nActionID(nActionID),
m_rSemaphore(rSemaphore),
m_pDB(pDB),
m_bProcessForCurrentUPIOnly(bProcessForCurrentUPIOnly)
{
}
//-------------------------------------------------------------------------------------------------
										...
										IFileProcessingDB* pFPDB, IFAMTagManager* pFAMTagManager)
{
	...
	try
	{
		...
		// Setup the work item threads if any tasks are set to parallelize, and set the work item
		// threads to only process workItems generated by this instance.
		bool bParallelize = setupWorkItemThreadData(nNumThreads, lActionID, true);
		if (bParallelize)
		{
...
void CFileProcessingMgmtRole::startProcessing(bool bDontStartThreads)
{
	...
	try
	{
		try
		{
			...
			bool bParallelize = setupWorkItemThreadData(nNumThreads, lActionID, false);			
	...
	if(bParallelize)
	{
		...
		for (int i = 0; i < nNumberOfThreads; i++ )
		{
			WorkItemThreadData *threadData = 
				new WorkItemThreadData(this, lActionID, *m_upParallelSemaphore.get(), m_pDB, bProcessForCurrentUPIOnly);
