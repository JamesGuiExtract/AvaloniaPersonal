...
static const string gstrCREATE_ACTION_STATISTICS_TABLE = "CREATE TABLE [dbo].[ActionStatistics]("
	...
	"CONSTRAINT [PK_Statistics] PRIMARY KEY CLUSTERED ([ActionID] ASC, [Deleted] ASC))";
...
static const string gstrRECREATE_ACTION_STATISTICS_FOR_ACTION = 
	...
	"	SELECT\r\n"
	"		DeletedOrNot.Deleted,\r\n"
	"		Action.ID AS ActionID,\r\n"
	"		COALESCE(WorkflowID, 0) AS WorkflowID\r\n"
	"	FROM (SELECT 0 AS Deleted UNION SELECT 1) AS DeletedOrNot, Action\r\n"
	"),\r\n"
	...
	"				AND WorkflowState.Deleted = WorkflowFileStatus.Deleted\r\n"
	...
	"				AND WorkflowState.Deleted = ActionStatistics.Deleted\r\n"
	"			WHERE ActionStatistics.ActionID IS NULL\r\n"
	"		GROUP BY WorkflowState.ActionID, WorkflowState.Deleted, WorkflowFileStatus.ActionStatus\r\n"
	"	) AS totals\r\n"
	"	GROUP BY ActionID, Deleted\r\n"
	") AS NewStats\r\n"
	"<ActionIDWhereClause>";
...
//		<DeletedFilesWhereClause> Should be replaced with 1 for deleted file stats or 0 for non-deleted file stats
static const string gstrCALCULATE_ACTION_STATISTICS_FOR_ACTION =
	...
	"	AND [ActionStatistics].[Deleted] = [ActionStatisticsDelta].[Deleted]\r\n"
	"WHERE [ActionStatistics].[ActionID] = <ActionIDWhereClause>\r\n"
	"	AND [ActionStatistics].[Deleted] = <DeletedFilesWhereClause>\r\n"
...
static const string gstrUPDATE_ACTION_STATISTICS_FOR_ACTION_FROM_DELTA =
	...
	"	GROUP BY ActionID, Deleted\r\n"
	") AS Changes\r\n"
	"ON ActionStatistics.ActionID = Changes.ActionID AND ActionStatistics.Deleted = Changes.Deleted";
...
"			)  \r\n"
"  \r\n"
"		DECLARE @SkippedFileJoin NVARCHAR(MAX) = ''''  \r\n"
"		DECLARE @SkippedFileWhere NVARCHAR(MAX) = ''''  \r\n"
...
"		IF (@StatusToQueue = ''S'')  \r\n"
"		BEGIN  \r\n"
"			SET @SkippedFileJoin = ''INNER JOIN SkippedFile WITH (NOLOCK) ON FileActionStatus.ActionID = SkippedFile.ActionID  \r\n"
"				AND FileActionStatus.FileID = SkippedFile.FileID ''  \r\n"
"			SET @SkippedFileWhere = '' AND (  \r\n"
"					@SkippedForUser = ''''''''  \r\n"
...
"		BEGIN  \r\n"
"			SET @WorkflowFileWhere = '' AND (WorkflowFile.WorkflowID IS NULL OR WorkflowFile.Deleted = 0) ''  \r\n"
"		END  \r\n"
"  \r\n"
...
"				,COALESCE(WorkflowFile.Deleted, 0) AS Deleted \r\n"
...
"			), Ranked AS \r\n"
"			( \r\n"
"				SELECT ActionID, FileID, ActionStatus, Priority, Deleted, \r\n"
"				RANK() OVER \r\n"
...
"		GROUP BY Selected.ActionID, Selected.Deleted  \r\n"
"	END TRY  \r\n"
"  \r\n"
"	BEGIN CATCH  \r\n"
"		DECLARE @ErrorMessage NVARCHAR(4000);  \r\n"
