...
static const string gstrMARK_TASK_SESSION_ATTRIBUTE_DATA_UNMODIFIED =
	...
	"	INNER JOIN [FileTaskSession] WITH (NOLOCK) ON [FileTaskSessionID] = [FileTaskSession].[ID] \r\n"
	"	INNER JOIN [FAMSession] WITH (NOLOCK) ON [FileTaskSession].[FAMSessionID] = [FAMSession].[ID] \r\n"
	"	INNER JOIN [ActiveFAM] WITH (NOLOCK) ON [ActiveFAM].[FAMSessionID] = [FAMSession].[ID] \r\n"
	"	WHERE [FileTaskSessionID] = <FileTaskSessionID> \r\n"
...
"-- If @ActionID is provided files returned will only be for that action"
"-- If @WorkflowID is not provided either no workflows exist or this is for all workflows \r\n"
"-- =============================================\r\n"
...
"			WHERE @ActionID = ID \r\n"
"				OR @ActionID IS NULL AND ASCName = @ActionName \r\n"
...
"						,READPAST\r\n"
"						)\r\n"
"				INNER JOIN @Actions A ON A.ActionID = FileActionStatus.ActionID \r\n"
...
"				LEFT JOIN WorkflowFile WITH (NOLOCK) ON WorkflowFile.FileID = FileActionStatus.FileID \r\n"
...
"					AND FileActionStatus.FileID\r\n"
"						NOT IN (SELECT FileID\r\n"
"								FROM FileActionStatus WITH(NOLOCK)\r\n"
"								WHERE FileActionStatus.ActionStatus = ''R''  AND FileActionStatus.ActionID NOT IN (SELECT ActionID FROM @Actions))\r\n"
...
"	INNER JOIN FAMFile  WITH (NOLOCK) ON FAMFile.ID = Selected.FileID\r\n"
"	ORDER BY Selected.Priority desc, Selected.FileID \r\n"
"END\r\n"
"')";
