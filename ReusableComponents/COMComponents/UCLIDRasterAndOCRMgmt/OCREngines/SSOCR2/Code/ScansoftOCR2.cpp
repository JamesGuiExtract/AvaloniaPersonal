...
long CScansoftOCR2::ms_lCurrentPageIndex;
  ...
  m_nTimeoutLength(120000)
{
	try
	{
		ASSERT_RESOURCE_ALLOCATION("ELI17066", m_ipSpatialString != NULL);
		...
		VARIANT_BOOL vbUpdateProgressStatus, EPageDecompositionMethod eDecompMethod, BSTR* pStream)
{
	...
	try
	{
		...
		ASSERT_RESOURCE_ALLOCATION("ELI11061", ipPageNumbers != NULL);
		...
		// set user zone and decomposition method if appropriate
		LPRECT lpRect = NULL;
		RECT rect;
		if(pZone)
		{
			...
			// a decomposition method is necessary to help identify the locations of handwritten 
			// characters, but not used if locating printed text within a zone
			if(bDetectHandwriting)
			{
				setDecompositionMethod(eDecompMethod);
			}
		}
		else
		{
		...
		ASSERT_RESOURCE_ALLOCATION("ELI18368", ipObj != NULL);
...
STDMETHODIMP CScansoftOCR2::SupportsTrainingFiles(VARIANT_BOOL *pbValue)
{
	...
	try
	{
		...
		ASSERT_ARGUMENT("ELI18371", pbValue != NULL)
...
STDMETHODIMP CScansoftOCR2::raw_IsLicensed(VARIANT_BOOL * pbValue)
{
	...
	try
	{
		ASSERT_ARGUMENT("ELI18326", pbValue != NULL);
...
{
	ASSERT_ARGUMENT( "ELI16661", ipPageInfo != NULL);
	...
	bool bReturnSpatialInfo)
{
	...
	ASSERT_RESOURCE_ALLOCATION("ELI15216", ipPageInfos != NULL);
	...
	vector<CPPLetter>* pvecLetters, ILongToObjectMapPtr ipPageInfos)
{
	...
	long lCount = vecPageNumbers.size();

	// recognize each page in sequence, accumulate the text, 
	// and return the total accumulated text
	for (ms_lCurrentPageIndex = 0; ms_lCurrentPageIndex < lCount; ms_lCurrentPageIndex++)
	{
		ms_lCurrentPageNumber = vecPageNumbers[ms_lCurrentPageIndex];
		...
		{
			// we should never reach here when ms_lCurrentPageIndex <= 1
	...
	vector<CPPLetter>* pvecLetters, const string& strText, const ILongToObjectMapPtr& ipPageInfos)
{
	...
	ASSERT_RESOURCE_ALLOCATION("ELI18370", m_ipSpatialString != NULL);
												 ...
												 long lVerticalDPI, bool bReturnUnrecognized)
{
	...
	ASSERT_ARGUMENT("ELI06187", pvecLetters != NULL);
...
RECERR __stdcall CScansoftOCR2::ProgressMon(LPPROGRESSMONITOR mon, void* pContext) // Static
{
	try
	{
		...
		if (ms_bUpdateProgressStatus)
		{
			...
			ms_lPageIndex = ms_lCurrentPageIndex;
	...
	string& rstrText, vector<CPPLetter>* pvecLetters, ILongToObjectMapPtr ipPageInfos)
{
	...
	ASSERT_RESOURCE_ALLOCATION("ELI12722", ipPageInfo != NULL);
	...
	if (pZone != NULL && !bDetectHandwriting)
	{
...
//-------------------------------------------------------------------------------------------------
void CScansoftOCR2::setDecompositionMethod(EPageDecompositionMethod eDecompMethod)
{
	IMG_DECOMP imgDecompMethod = DCM_AUTO;

	switch (eDecompMethod)
	{
	case kAutoDecomposition:
		imgDecompMethod = DCM_AUTO;
		break;
	case kStandardDecomposition:
		imgDecompMethod = DCM_STANDARD;
		break;
	case kLegacyDecomposition:
		imgDecompMethod = DCM_LEGACY;
		break;
	default:
	...
	THROW_UE_ON_ERROR("ELI06368", "Unable to set the page decomposition method in the OCR engine!",
		kRecSetDecompMethod(0, imgDecompMethod));
}
//-------------------------------------------------------------------------------------------------
