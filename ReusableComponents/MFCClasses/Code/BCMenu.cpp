...
BCMenu::~BCMenu()
{
	...
	CATCH_AND_LOG_ALL_EXCEPTIONS("ELI16534");
}
...
BCMenuData::~BCMenuData()
{
	...
	CATCH_AND_LOG_ALL_EXCEPTIONS("ELI16496");
}
...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		long lLength = wcslen(szWideString)+1;
		m_szMenuText = new wchar_t[lLength];
		if (m_szMenuText)
			wcscpy_s(m_szMenuText, lLength, szWideString);
    }
	else
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		nLen = 0;
		long lLength = wcslen((wchar_t*)pTp)+1;
		szCaption=new wchar_t[lLength];
		wcscpy_s(szCaption, lLength, (wchar_t*)pTp);
