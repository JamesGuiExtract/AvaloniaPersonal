//**** test driver, angle_test.cpp file****
...
void test_from_keyboard();           //get angle string or radians from keyboard
void test_from_file(string strOption, char* fileName);   //get angle string or radians from file

void get_angle();          //get angle from keyboard
void get_radians();          //get radians from keyboard
void test_angle(Angle  angleA, string strText); //test angle which get from keyboard
void test_radians(Angle  angleA, double angle);   //test radians which get from keyboard
bool compare_vec(vector<string> vec_1,   vector<string>  vec_2); //vec_1 <= vec_2, return true
...
int main(int argc, char*argv[])
{
	...
	cout<<endl<<"************    Test Angle class    ************"<<endl;

	string command;   //"keyboard", "-a", "-r" are valid command
	...
	else if(argc == 3)  //command must be "-k" or "-r", otherwise invalid
	{
		command = argv[1];
...
void help_message()
{
	//print help message
		...
		cout<<"There options for testing the Angle class."<<endl;
		cout<<"1) Test angle string from the file."<<endl
			<<"   Type .exe file followed by option '-a' and the input file name.\n"
			<<"   example  ...> angle_test -b t1.txt "<<endl;
		cout<<"2) Test radians from the file."<<endl
			<<"   Type .exe file followed by option '-r' and the input file name.\n"
			<<"   example  ...> angle_test -r t2.txt "<<endl;
		cout<<"3) Test angle string or radians from key board."<<endl
			<<"   Type .exe file followed by ' -k '\n"
			<<"   example  ...> angle_test -k "<<endl;
}
...
void test_from_keyboard()
{
	...
	while(1)
	{
		cout<<" **** Test angle string or radians from key board ****"<<endl;
...
//-----------------------------------------------------------------------------------------
//Get angle string from keyboard. Since i use getline(cin....), may be some thing wrong in
//library, user must hit "Enter" twice.
void get_angle()
{
	//get input string from keyboard
	cout<<"  **** TEST angle string, get input string from key board ****"<<endl;
	...
	while(1)
	{
		...
		if(strText == strChangeMode)
			angleA1.toggleReverseMode();
		else
			test_angle(angleA1, strText);
...
void get_radians()
{
	...
	while(1)
	{
		...
		if(angle == -111)
			break;
		if(angle == -11)
			angleA4.toggleReverseMode();
		else
			test_radians(angleA4, angle);	 
	}
}
//----------------------------------------------------------------------------------------

//----------------------------------------------------------------------------------------
//Test angle string, call evaluate(..) function, cout result
void test_angle(Angle  angleA, string strText)
{
	...
	try
	{
		angleA.evaluate(strText.c_str());
		cout<<"                    \""<<angleA.asString()<<"\""<<endl;
		cout<<"                     "<<angleA.getDegrees()<<endl;
	}
	...
	if(!angleA.isValid())
	{
		...
		vec_string = angleA.getAlternateStrings();
...
void test_radians(Angle angleA, double angle)
{
...
//-----------------------------------------------------------------------------------------
//Get angle string or radians from file.
//If option is "-a", open angle string file and write the result to "out1.txt".
...
void test_from_file(string strOption, char *fileName)
{
	...
	if(strOption == "-a")
	{
		cout<<"  **** TEST , get input angle from file  ****"<<endl;
	...
	while(!fin.eof())
	{
		...
		//if strOption is "-a", evaluate the angle string
		//if strOption is "-r", evaluate the radians, when evaluate the radians, 
		//if the radians value is not valid (<0 or > 2*PI), the efvaluatRadians(dValue)
		...
		if(strInput != "")  //avoid empty line
		{
			...
			if(strOption == "-a")
				angleA2.evaluate(strInput.c_str());
			else if(strOption == "-r")
				angleA2.evaluateRadians(atof(strInput.c_str()));
			...
			//check string
			if(!angleA2.isValid())  //bearing string is not valid
			{
				...
				vector<string> vec_string;
				vec_string = angleA2.getAlternateStrings();
				bool compare = true;
				...
				else if(strAlternate != "NONE" && !compare_vec(vec_alt_str, vec_string))
				{
					compare = false;
					iNumFail++;
					iNoMatchAltStr++;
			...
			else  //string is valid, check if it match the file
			{
				...
				if(strStand != "NONE" && strStand != angleA2.asString())  //valid, then check match standard string
				{
					iNumFail++;
					...
					fout<<      "    Standard string calculated = "<<angleA2.asString()<<endl;
					
				}else  //match stand string, keep checking angle
				{
					double dTemp1 = angleA2.getDegrees();
					...
					if(strAngle != "NONE" && fabs(dTemp1 - dTemp2)>0.02) 
					{
						...
						fout<<      "    Angle string calculated = "<<angleA2.getDegrees()<<endl;
					...
					else //match the angle, check alternate string should be standard string
					{
						...
						if(strAlternate != "NONE" && !compare_vec(vec_alt_str, vec_string))
						{
							iNumFail++;
							iNoMatchAltStr++;
...
bool compare_vec(vector<string> vec_1,  vector<string> vec_2)
{
	bool bFlag = false;
