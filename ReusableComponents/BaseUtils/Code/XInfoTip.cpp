...
CXInfoTip::~CXInfoTip()
{
	...
	CATCH_AND_LOG_ALL_EXCEPTIONS("ELI16426");
}
...
BOOL CXInfoTip::Create(CWnd* pParentWnd) 
{
	...
	//			(for some reason, m_szClass doesn't work for ATL property pages)
	// bSuccess = CreateEx(NULL, m_szClass, NULL, WS_POPUP, 0, 0, 0, 0, pParentWnd->GetSafeHwnd(), NULL, NULL);
	bSuccess = CreateEx(NULL, AfxRegisterWndClass(0), NULL, WS_POPUP, 0, 0, 0, 0, pParentWnd->GetSafeHwnd(), NULL, NULL);
	// END DUAN'S MODIFICATION 08/04/2003
...
void CXInfoTip::OnPaint() 
{
	...
	// Create the brushes
	// BEGIN ARVIND'S MODIFICATIONS 08/05/2003
	// PURPOSE: To change background color of tip window to light yellow
	// InnerFrameBrush.CreateSolidBrush(::GetSysColor(COLOR_SCROLLBAR));
	COLORREF lightYellowColor = RGB(255, 255, 210);
	InnerFrameBrush.CreateSolidBrush(lightYellowColor);
	// END ARVIND'S MODIFICATIONS
	...
	WindowBrush.CreateSolidBrush(lightYellowColor);
	// END ARVIND'S MODIFICATIONS
