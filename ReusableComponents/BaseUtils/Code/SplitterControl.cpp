...
CSplitterControl::~CSplitterControl()
{
	...
	CATCH_AND_LOG_ALL_EXCEPTIONS("ELI16399");
}
...
void CSplitterControl::DrawLine(int /*nX*/, int /*nY*/)
{
	CWindowDC dc(NULL);
	...
	if (m_nType == SPS_VERTICAL)
	{
		dc.MoveTo(m_nX - d, rcWnd.top+5);
		dc.LineTo(m_nX - d, rcWnd.bottom-5);
	...
	{
		dc.MoveTo(rcWnd.left+5, m_nY - d);
		dc.LineTo(rcWnd.right-5, m_nY - d);
...
/*	CRect rcWnd;
	...
	if (m_nType == SPS_VERTICAL)
	{
		dc.MoveTo(m_nX - d, rcWnd.top);
		dc.LineTo(m_nX - d, rcWnd.bottom);
	...
	{
		dc.MoveTo(rcWnd.left, m_nY - d);
		dc.LineTo(rcWnd.right, m_nY - d);
...
void CSplitterControl::ResizeSplitter(CRect rect)
{
	CRect rc;
	GetWindowRect(rc);
