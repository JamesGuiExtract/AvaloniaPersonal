...
// Gets a local named mutex object. If strMutexName does not start with
// the word "Local\" then "Local\" will be prepended to the mutex name.
// This method will return a mutex that has its security settings set
// such that all users (the "Everyone" SID) have synhchronize and modify
// access to the mutex.
EXPORT_BaseUtils CMutex* getLocalNamedMutex(string strMutexName);
