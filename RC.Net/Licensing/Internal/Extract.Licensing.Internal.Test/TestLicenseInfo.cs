...
namespace Extract.Licensing.Internal.Test
{
    ...
    public class TestLicenseInfo
    {
        ...
        // Unlock code ComputerName: VOYAGER, DiskSerial number: 0ADD36E4, MAC ADDRESS 3417EBA0BF7D, expire date 8/25/2020
        const string UnlockCode = "C95F207ACCC7F6391AA1245EB4C17755724B457710C0A95C23D4287934852E18713A8BFF0D7EF614";
        ...
        [Category("Automated")]
        [Category("Internal")]
        public void TestUserLicenseStringGeneration()
        {
            var licenseInfo = new LicenseInfo();
            licenseInfo.UserString = LicenseInfo.GenerateUserString();
            Assert.AreEqual(Environment.MachineName.ToUpper(), licenseInfo.UserComputerName.ToUpper());
            var firstMacAddress = NetworkInterface.GetAllNetworkInterfaces()
                .Where(nic => nic.OperationalStatus == OperationalStatus.Up &&
                    nic.NetworkInterfaceType != NetworkInterfaceType.Loopback)
                .Select(nic => nic.GetPhysicalAddress().ToString())
                .FirstOrDefault();
            Assert.AreEqual(firstMacAddress.ToUpper(), licenseInfo.UserMACAddress.ToUpper());
            Assert.AreEqual((UInt32)Extract.Utilities.UtilityMethods.GetVolumeSerialNumber("C:\\"), licenseInfo.UserSerialNumber);
        }
        ...
        [Category("Automated")]
        [Category("Internal")]
        public void TestUnlockCodeGeneration()
        {
            var licenseInfo = new LicenseInfo();
            licenseInfo.UserString = LicenseInfo.GenerateUserString();
            ...
            Assert.AreEqual(licenseInfo.UserComputerName.ToUpper(), expandedUnlockInfo.UserComputerName.ToUpper());

            Assert.AreEqual(licenseInfo.UserMACAddress.ToUpper(), expandedUnlockInfo.UserMACAddress.ToUpper());
            Assert.AreEqual(licenseInfo.UserSerialNumber, expandedUnlockInfo.UserSerialNumber);
            Assert.AreEqual(expire, expireDate);
        }
        ...
        public void TestUnlockCode()
        {
            ...
            Assert.AreEqual("VOYAGER", expandedUnlockInfo.UserComputerName.ToUpper());

            Assert.AreEqual("3417EBA0BF7D", expandedUnlockInfo.UserMACAddress.ToUpper());
            Assert.AreEqual(0x0ADD36E4, expandedUnlockInfo.UserSerialNumber);
            Assert.AreEqual(DateTime.Parse("9/24/2020"), expireDate.ToLocalTime().Date);
        }
