...
namespace Extract.FAMDBCounterManager
{
    ...
    /// values are changed.
    /// <para><b>Note</b></para>
    /// This class is a modified copy of Extract.Utilities.ConfigSettings. This project is not
    ...
    internal sealed class ConfigSettings<T> : ExtractSettingsBase<T> where T : ApplicationSettingsBase, new()
    {
            ...
            : base(dynamic)
        {
            ...
            if (configFileName == null)
            {
                ...
                _configFileName = Path.GetFileName(Assembly.GetAssembly(typeof(T)).Location) + ".config";
            }
            else
            {
        ...
        ClientSettingsSection GetConfigSection(bool userSection, bool recursing = false)
        {
            ...
            UtilityMethods.Assert(configSectionGroup != null, "Settings group missing");
            ...
            UtilityMethods.Assert(configSection != null, "Settings section missing");
                ...
                if (configSection != null)
                {
                    ...
                    foreach (SettingsProperty setting in Settings.Properties)
                    {
                        ...
                        if (xmlSetting != null)
                        {
                            ...
                            string stringValue =
                                xmlSetting.Value.ValueXml.GetNodeValue(false);
                            ...
                            UpdatePropertyFromString(settingName, stringValue);
                        }
                    }
                }
            }
        ...
        void ValidateSettings(ClientSettingsSection configSection)
        {
            ...
            UtilityMethods.Assert(!invalidXmlSettings.Any(), "Invalid Application Setting(s)");
        }
        ...
        void ProcessObjectSettings(XmlNodeList nodes, object instance)
        {
            ...
            foreach (XmlNode node in nodes)
            {
                ...
                if (node.Name.Equals("Member", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    UtilityMethods.Assert(field != null && field.GetType().IsClass, "Unknown member");
                ...
                else if (node.Name.Equals("Property", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    UtilityMethods.Assert(objectProperty != null, "Unknown property");
                    ...
                    string stringValue = node.GetNodeValue(true);
        ...
        static string GetXmlNameAttribute(XmlNode node)
        {
            ...
            UtilityMethods.Assert(attribute != null, "XML node name attribute missing");
    ...
    /// <para><b>Note</b></para>
    /// This class is a modified copy of Extract.Utilities.ConfigSettingsExtensionMethods. This
    /// project is not linked to Extract.Utilities to avoid COM dependencies.
    /// </summary>
    internal static class ConfigSettingsExtensionMethods
    {
        /// <summary>
        ...
        public static string GetNodeValue(this XmlNode node, bool xmlDefault)
        {
