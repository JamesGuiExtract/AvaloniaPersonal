...
module RuleExecutionCounters =
  ...
  let setFromDto (target: IRuleSet) (dto: Counter list) =
    ...
    |> Seq.iter (fun counter ->
      ...
      | _ -> customCounters.PushBack ([box counter.ID; box counter.Name; box counter.ByPage; box counter.Enabled].ToVariantVector ())
    )
    if customCounters.Size () > 0
    then target.CustomCounters <- customCounters
...
module OCRParameters =
  ...
  let toDto (domain: IOCRParameters) =
    ...
    |> VariantVector.toSeq
    |> Seq.map (fun (vp: IVariantPair) -> KeyValuePair<_,_> (vp.VariantKey |> changeIntToLong, vp.VariantValue |> changeIntToLong))
    |> Seq.toList

  let fromDto (dto: KeyValuePair<_,_> list) =
    dto
    |> Seq.map (fun kv -> VariantPairClass(VariantKey=(kv.Key |> changeLongToInt), VariantValue=(kv.Value |> changeLongToInt)))
...
module AttributeNameToInfoMap =
  ...
  let fromDto mc (dto: IDictionary<string, Dto.AttributeFindInfo>) =
    ...
    dto |> Seq.iter (fun kv -> domain.Set (kv.Key, (kv.Value |> AttributeFindInfo.fromDto mc)))
    domain :?> 't
...
module RuleSet =
  ...
  let toDto mc (domain: IRuleSet): Dto.RuleSet =
    ...
    { SavedWithSoftwareVersion = Dto.RuleSet.version
      ...
      OCRParameters = ((domain :?> IHasOCRParameters).OCRParameters |> OCRParameters.toDto)
      ...
      GlobalDocPreprocessor = domain.GlobalDocPreprocessor |> ObjectWithDescription.toDto mc
      IgnorePreprocessorErrors = domain.IgnorePreprocessorErrors
      AttributeNameToInfoMap = domain.AttributeNameToInfoMap |> AttributeNameToInfoMap.toDto mc
      GlobalOutputHandler = domain.GlobalOutputHandler |> ObjectWithDescription.toDto mc
    ...
    let domain =
      RuleSetClass
        ( Comments=dto.Comments,
          ...
          GlobalDocPreprocessor=(dto.GlobalDocPreprocessor |> ObjectWithDescription.fromDto mc),
          IgnorePreprocessorErrors=dto.IgnorePreprocessorErrors,
          AttributeNameToInfoMap=(dto.AttributeNameToInfoMap |> AttributeNameToInfoMap.fromDto mc),
          GlobalOutputHandler=(dto.GlobalOutputHandler |> ObjectWithDescription.fromDto mc),
